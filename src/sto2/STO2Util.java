...
public class STO2Util {
	...
	// theta means p( z| d) but for each sentment level
	public static DoubleMatrix [] calculateTheta(int d, IntegerMatrix [] matrixSDT, int [][] sumDST, double alpha, double sumAlpha) {
		int numSenti = matrixSDT.length;
		int numDocs = matrixSDT[0].getNumOfRow();
		int numTopics = matrixSDT[0].getNumOfColumn();
	...
	// pi means p( sentiment | d) 
	public static DoubleMatrix calculatePi(int d, IntegerMatrix matrixDS, int [] sumDS, double [] gammas, double sumGamma) {
		int numDocs = matrixDS.getNumOfRow();
		int numSenti = matrixDS.getNumOfColumn();
